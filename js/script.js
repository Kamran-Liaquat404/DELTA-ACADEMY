...
function initImageFallbacks() {
  ...
  images.forEach((img) => {
    img.addEventListener('error', () => {
      ...
      if (wrapper && wrapper.classList.contains('teacher-image-wrapper')) {
        // Fallback for teacher image
        wrapper.style.display = 'flex';
        wrapper.style.alignItems = 'center';
        wrapper.style.justifyContent = 'center';
        wrapper.style.backgroundColor = '#1D4ED8';
        ...
        wrapper.innerHTML = `<div style="text-align: center;">
          ...
          <div style="font-size: 0.8rem; font-weight: 600; opacity: 0.8; text-transform: uppercase;">DELTA Faculty</div>
        </div>`;
      }
    });
  });
