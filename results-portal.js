...
(function () {
  ...
  async function initPortal() {
    ...
    // Expose global helper to load any roll number
    window.loadStudentResult = function (roll) {
      if (input) {
        input.value = roll;
        handleSearch(roll);
      }
  ...
  function renderResultCard(s, container) {
    ...
    const mcqs = Number(s.mcqsMarks || 0);
    const diagram = Number(s.diagramMarks || 0);
    const shortQ = Number(s.shortQuestionsMarks || 0);
    const mathMarks = Number(s.mathMarks || 0);
    const mathTotal = Number(s.mathTotal || (String(s.class).includes('6th') ? 50 : 40));

    // Grand total including Mathematics
    const grandTotal = 100 + mathTotal;
    const totalObtained = Number((mcqs + diagram + shortQ + mathMarks).toFixed(1));
    const percentage = Number(((totalObtained / grandTotal) * 100).toFixed(1));
    ...
    const grade = s.grade && !s.mathMarks ? s.grade : getGrade(percentage);
    ...
    container.innerHTML = `
      <div class="clean-result-card" id="printable-result-sheet">
        ...
        <div class="clean-result-header">
          ...
          <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 0.4rem;">
            <div class="clean-result-tag">
              <span>DELTA Academy Examination &bull; ${escapeHtml(s.class)}</span>
            </div>
        ...
        <!-- Paper Components & Subject Table -->
        <div class="clean-table-wrap">
          <table class="clean-marks-table">
            <thead>
              <tr>
                <th style="text-align: left;">Components / Subject</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style="text-align: center;">${mathTotal}</td>
                <td style="text-align: center;" class="score-highlight">${s.mathMarks}</td>
              </tr>
            </tbody>
            <tfoot>
              <tr class="clean-total-row">
                ...
                <td style="text-align: center; font-weight: 800;" class="total-score-highlight">${totalObtained}</td>
              </tr>
            </tfoot>
          </table>
        </div>

        <!-- Bottom Total Score Area (Including Mathematics) -->
        <div class="clean-summary-strip">
          <div class="clean-summary-item">
            <span class="summary-lbl">Total Marks</span>
            <span class="summary-val">${grandTotal}</span>
          </div>
          <div class="clean-summary-item highlight">
            <span class="summary-lbl">Obtained Marks</span>
            <span class="summary-val">${totalObtained}</span>
          </div>
          <div class="clean-summary-item">
          ...
          <div class="clean-summary-item">
            ...
            <span class="summary-val">${escapeHtml(grade)}</span>
          </div>
          <div class="clean-summary-item">
            <span class="summary-lbl">Result Status</span>
